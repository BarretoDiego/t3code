...
export function $setComposerEditorPrompt(
  prompt: string,
  terminalContextsOrSkillMetadata:
    | ReadonlyArray<TerminalContextDraft>
    | ReadonlyMap<string, ComposerSkillMetadata>,
  maybeSkillMetadata?: ReadonlyMap<string, ComposerSkillMetadata>,
): void {
  const root = $getRoot();
  root.clear();

  const terminalContexts = maybeSkillMetadata ? terminalContextsOrSkillMetadata : [];
  const skillMetadata = (maybeSkillMetadata ?? terminalContextsOrSkillMetadata) as ReadonlyMap<
    string,
    ComposerSkillMetadata
  >;
  const segments = splitPromptIntoComposerSegments(
    prompt,
    terminalContexts as ReadonlyArray<TerminalContextDraft>,
  );
  ...
  for (const segment of segments) {
    if (segment.type === "citation") {
      currentParagraph().append($createComposerCitationNode(segment.citation, segment.source));
      continue;
    }
    ...
    if (segment.type === "mention") {
      currentParagraph().append($createComposerMentionNode(segment.path, segment.source));
      continue;
    }
