...
export const make = Effect.gen(function* () {
  ...
  const execute: GitHubCli["Service"]["execute"] = Effect.fn("GitHubCli.execute")(
    function* (input) {
      ...
      // Account settings remain the fallback for source-control hub operations. A routing
      // credential is more specific: it has already been verified for this request's host.
      const account =
        credential === null && Option.isSome(accounts)
          ...
          : undefined;
      const token =
        credential === null ? account?.token || undefined : Redacted.value(credential.token);
      const env =
        token === undefined
          ? input.env
          : {
              ...input.env,
              ...(credential === null ? {} : { GH_HOST: credential.host }),
              GH_TOKEN: token,
              GITHUB_TOKEN: token,
