...
One metered period inside a plan rate-limit observation: a label, a used percentage, and the instant it rolls over. Windows merge by id, so a sparse update from a provider never deletes a window it did not mention.
